...
//caching buffer
static uint32_t flash_buffer[FLASH_ADDR_MAX];  // holds all values read from flash
...
// --- CS Pin Setup (PA15) ---
#define W25QXX_CS_PORT FLASH_SPI1_CS_GPIO_Port
#define W25QXX_CS_PIN  FLASH_SPI1_CS_Pin
...
{
    if ((start_addr + count) > FLASH_ADDR_MAX)
        return 1;
...
void flush_buffer_dump(void){
	dbg_print("memory values -----------------------------\n");
	for (uint32_t i = 0; i < FLASH_ADDR_MAX; ++i) {
	        dbg_print("%lu --- 0x%08lX\r\n", i, flash_buffer[i]);
	    }
}
