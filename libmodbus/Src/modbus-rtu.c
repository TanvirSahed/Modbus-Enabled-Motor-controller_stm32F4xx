...
static int _modbus_rtu_connect(modbus_t *ctx) {
	...
	/*Initialization with &huart, baudrate, parity, data_bit, stop_bit
	 * 		   ctx_rtu->device,
	 ctx_rtu->baud,
	 ctx_rtu->parity,
	 ctx_rtu->data_bit,
...
MODBUS_RTU_MAX_ADU_LENGTH, _modbus_set_slave, _modbus_rtu_build_request_basis,
		...
		_modbus_rtu_send, //this
		_modbus_rtu_receive,
		_modbus_rtu_recv, //this
...
modbus_t*
modbus_new_rtu(const char *device, int baud, char parity, int data_bit,
		int stop_bit) {
	modbus_t *ctx;
	modbus_rtu_t *ctx_rtu;
